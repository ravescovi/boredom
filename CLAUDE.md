...
**Database.** Prisma **7** schema in `packages/database/prisma/schema.prisma` (Postgres). Uses the `prisma-client` generator (output to `packages/database/src/generated/prisma`, gitignored) and the **pg driver adapter** — no engine binary. The runtime client (`packages/database/src/index.ts`) connects via `PrismaPg` over the pooled `DATABASE_URL`. Connection/migration config lives in `packages/database/prisma.config.ts` (not in `schema.prisma`), which loads `.env` via dotenv. `GameVersion` is immutable and stores `specJson` plus the optional rendered `markdownSnapshot`. `GenerationEvent` is the audit/quota record; `rejectionReasons` is a `String[]` populated from the validator's failure reasons.

**Migrations on deploy.** Production deploys auto-apply pending migrations: the `apps/web` build script runs `prisma migrate deploy --config=../../packages/database/prisma.config.ts` when `VERCEL_ENV=production` (CI/preview/local builds skip it, keeping `build` DB-free). Migrations connect over the direct/unpooled `DATABASE_URL_UNPOOLED` (set as the `prisma.config.ts` datasource URL), since Neon's pooler is unreliable for DDL. Add a migration with `pnpm --filter @bordon-ai/database db:migrate`; do **not** rely on `prisma db push`. See `docs/decisions/0003-prod-db-reset-and-auto-migrate.md`.
